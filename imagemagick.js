...
exports.identify = function(pathOrArgs, callback) {
	...
	var isCustom = Array.isArray(pathOrArgs),
		args = (Array.isArray(pathOrArgs)) ? parseArgObjs(pathOrArgs) : ['-verbose', pathOrArgs];
...
exports.convert = function(args, timeout, callback) {
	// get the args in the proper format
	var reArgs = parseArgObjs(args);
	...
	return exec2(exports.convert.path, reArgs, procopt, callback);
};
...
/***********************************/
/****** ADD'L HELPER FUNCTIONS *****/
/***********************************/
// hashmap of commands / instructions that should be hyphenated in the command-line style
var hyphenations = {
	density: "-density",
	format: "-format",
	resize: "-resize",
...
/**
 * Private Function parseArgObjs
 * @param args {array} strings of commands and colon-separated name/value pairs
 * @returns {array} formatted strings for imagemagick command arguments
 *
 * the name value pairs help organizational logic and construction of the arguments,
 * but the array of single pairs allows for them to appear in an explicit order
 */
function parseArgObjs(args) {
	var obj, prop, val;
	var argList = [];
	for (var i = 0, l = args.length; i < l; i++) {

		// get the last character of the argumemnt
		var trailing = args[i].slice(-1);

		// as long as that char is not : or -
		if (trailing !== ":" && trailing !== "-") {
			// split the pair if there's splittin' to do
			pair = args[i].split(":");

			// check if it needs to be hyphened as a command
			pair[0] = hyphenateCommands(pair[0]);
		}

		// add the args to the command
		argList = argList.concat(pair);
	}
...
/**
 * Private Function hyphenateCommands
 * @param cmd { string }
 * @returns { string }
 *
 * returns either the original string, prepended by a hyphen if need be
 */
function hyphenateCommands(cmd) {
	// if it's already hyphenated, nuff said
	if (cmd.charAt(0) === "-") { return cmd; }

	// return the command, checked against the hash map
	return hyphenations[cmd] || cmd;
}
